...
- 결정론을 깨는 변경 금지: 동일 입력은 동일 SVG. seed·colorway·정렬 순서에 의존하는
  로직을 추가할 때 비결정 요소(현재 시각, 무작위, dict 순서 등)를 끌어들이지 말 것.
- 테스트 실패 시 반환값·mock·테스트 코드를 임의로 고쳐 통과시키지 말 것 — 로직 버그 신호로 간주한다.
- 앱 런타임은 DDL을 실행하지 않는다. 스키마 변경은 모노레포 마이그레이션으로만.
