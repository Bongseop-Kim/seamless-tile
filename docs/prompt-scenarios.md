...
### E1. 2x2 미러(만화경)
- **prompt**: `Kaleidoscope-like pattern with 2x2 mirror symmetry, mirrored in both directions, two colors.`
- **확인**: `intent.symmetry.kind == "mirror_2x2"` · 합성 SVG가 미러 타일 · seamless 유지

### E2. 수평 미러
- **prompt**: `Pattern mirrored horizontally (left-right reflection), simple two-color motif.`
- **확인**: `symmetry.kind == "mirror_h"`

### E3. 글라이드 반사 (선택)
- **prompt**: `Motif row with a glide-reflection repeat, single color.`
- **확인**: `symmetry.kind ∈ {"glide_h","glide_v"}` · `shift_mm`가 `tile_mm` 약수(미지정 시 기본 tile_mm/2)
...
1. 🟢 A → D → E → F2/F3 → H → I (Recraft 무관, 저비용)
