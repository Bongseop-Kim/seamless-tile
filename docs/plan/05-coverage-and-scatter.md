...
`engine/placement/path_following.py`(wave 확장),
`tests/test_lattice.py`·`tests/test_scatter.py`(신규).
...
- 범위가 큰 세션이다(전략 3종 + 곡선 lane). 필요시 5a(lattice/point_set/scatter)·
  5b(곡선 lane)로 분할 가능.
